"""Deploy v43 with encoding-fixed frontend."""
...
front_img = next(img['image'] for img in images if 'frontend-encoding-fix' in img['image'])
...
with open('deploy_payload_v43.json', 'w') as f:
    json.dump({'containers': containers, 'publicEndpoint': deployment['publicEndpoint']}, f)

sub2 = subprocess.run(['aws', 'lightsail', 'create-container-service-deployment', '--service-name', 'daiso-search-service', '--cli-input-json', 'file://deploy_payload_v43.json'], capture_output=True, text=True)
