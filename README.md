...
│   └── main.py           # 애플리케이션 진입점 (Main)
├── backend/              # 핵심 로직 및 인프라
│   ├── api/              # API 엔드포인트 (Router)
│   ├── services/         # 비즈니스 로직 (검색, STT, 파이프라인)
│   ├── search/           # 검색 인프라 (Adapters - Elastic, Qdrant)
│   ├── stt/              # STT 인프라 (Adapters - Google, Whisper)
│   └── database/         # 데이터베이스 및 모델
├── frontend/             # 프론트엔드 애플리케이션
│   ├── kiosk/            # 키오스크 UI (태블릿용)
│   ├── mobile/           # 모바일 지도/AR 화면
│   └── assets/           # 공통 리소스
└── poc/                  # 개념 증명(PoC) 및 레거시 코드
    ├── scripts/          # 유틸리티 및 유지보수 스크립트
    ├── experiments/      # 실험 로그 및 모델 테스트
    └── ...
```
