# backend/api_legacy_backup.py
"""
[백업] 원본 backend/api.py - 리팩토링 이전 버전
backend/api/ 디렉토리와 충돌하여 백업됨
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"Whisper adapter failed to load: {e}")
        print("STT endpoint will return simulation results")
    ...
    try:
        ...
        if whisper_adapter:
            ...
            stt_result = STTResult(
                text_raw="(simulation mode - Whisper not loaded)",
                confidence=0.8,
                lang="ko",
                latency_ms=100,
        ...
        if quality_result.status == "OK":
            ...
            if policy_intent.intent_type == "FIXED_LOCATION":
                ...
                if not final_response:
                    final_response = f"'{policy_intent.location_target}' location guide."
                    
            elif policy_intent.intent_type == "UNSUPPORTED":
                final_response = pg_config.get(
                    "fallback_message", 
                    "This service helps with product and store location guidance."
                )
            else:  # PRODUCT_SEARCH
                final_response = f"[PRODUCT_SEARCH] '{stt_result.text_raw}' search pending"
            ...
            final_response = "Voice recognition failed. Please try again."
