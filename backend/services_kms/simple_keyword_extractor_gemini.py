...
def extract_keyword(query):
    ...
    하지만, **'배수구뻥', '매직블럭', 'WD-40'** 처럼 고유명사화 된 제품명은 그대로(또는 가장 널리 쓰이는 명칭으로) 출력해야 정확한 검색이 됩니다.
    검색이 가능한 **'가장 구체적인 카테고리명'**이나 **'통용되는 대표 상품명'**을 출력하세요.

    # [Thinking Strategy: 2-Step Classification]
    1. **구체적 지명 여부**: 사용자가 특정 제품(예: 배수구뻥)을 지칭했는가? -> 그대로 유지.
    2. **대분류/중분류 판단**: 모호한 경우(예: 청소하는거) -> 다이소몰 공식 카테고리/표준명으로 변환.
    ...
    (C) Specificity Priority: "배수구뻥" 입력 시 "배수구클리너"로 뭉뚱그리지 말고 "배수구뻥" (또는 압축뚫어뻥) 등 구체적 명칭 우선.
