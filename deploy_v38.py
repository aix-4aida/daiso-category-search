"""Deploy section-fix to Lightsail."""
...
back_img = next(img['image'] for img in images if 'backend-section-fix' in img['image'])
...
with open('deploy_payload_v38.json', 'w') as f:
    json.dump({'containers': containers, 'publicEndpoint': deployment['publicEndpoint']}, f)

sub2 = subprocess.run(['aws', 'lightsail', 'create-container-service-deployment', '--service-name', 'daiso-search-service', '--cli-input-json', 'file://deploy_payload_v38.json'], capture_output=True, text=True)
