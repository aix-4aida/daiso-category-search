"""Deploy v44 with all fixes: corridor waypoints, score gap, QR, label."""
...
front_img = next(img['image'] for img in images if 'frontend-corridor-fix' in img['image'])
...
with open('deploy_payload_v44.json', 'w') as f:
    json.dump({'containers': containers, 'publicEndpoint': deployment['publicEndpoint']}, f)

sub2 = subprocess.run(['aws', 'lightsail', 'create-container-service-deployment', '--service-name', 'daiso-search-service', '--cli-input-json', 'file://deploy_payload_v44.json'], capture_output=True, text=True)
