...
const VoiceSearch = () => {
    ...
    const handleSearch = (text) => {
        ...
        setTimeout(() => {
            navigate('/map', { state: { voiceQuery: query } });
        }, 800); // Slight delay to show the final text
    };
...
/* 
// ==========================================
// 백업: 기존 파일 업로드 방식 (File Upload Logic)
// ==========================================

const VoiceSearch_Backup = () => {
    const navigate = useNavigate();
    const [isRecording, setIsRecording] = useState(false);
    const [transcript, setTranscript] = useState("");
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            ...
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
            const response = await fetch(`${apiUrl}/stt/process`, {
                method: "POST",
                body: formData,
            });
            ...
            if (data.text) {
                setTimeout(() => {
                    navigate('/map', { state: { voiceQuery: data.text } });
                }, 1000);
            }
        } catch (error) {
