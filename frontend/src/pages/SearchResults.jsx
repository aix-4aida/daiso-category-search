...
const SearchResults = () => {
    ...
    const [results, setResults] = useState(passedProducts || [])
    const [loading, setLoading] = useState(!passedProducts)
    const [selectedProduct, setSelectedProduct] = useState(null)

    useEffect(() => {
        if (passedProducts) {
            setResults(passedProducts)
            if (passedProducts.length > 0) setSelectedProduct(passedProducts[0])
            setLoading(false)
            return
        }
        ...
        return (
            <Layout className="bg-white">
                <header className="p-4 border-b flex items-center">
                    <button onClick={() => navigate('/')} className="mr-4"><ArrowLeft /></button>
    ...
    return (
        <Layout className="bg-gray-50 h-screen overflow-hidden flex flex-col">
            ...
            <header className="bg-white border-b px-6 py-3 flex items-center shrink-0">
                <button onClick={() => navigate('/')} className="mr-4 text-gray-600 hover:text-daiso-red transition-colors">
                    <ArrowLeft size={24} />
                </button>
