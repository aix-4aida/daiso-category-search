"""Deploy v41 with frontend cache fix."""
...
front_img = next(img['image'] for img in images if 'frontend-cache-fix' in img['image'])
...
with open('deploy_payload_v41.json', 'w') as f:
    json.dump({'containers': containers, 'publicEndpoint': deployment['publicEndpoint']}, f)

sub2 = subprocess.run(['aws', 'lightsail', 'create-container-service-deployment', '--service-name', 'daiso-search-service', '--cli-input-json', 'file://deploy_payload_v41.json'], capture_output=True, text=True)
